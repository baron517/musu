...
Page({
    ...
    loadData: function (options) {
        ...
        getApp().request({
            url: getApp().api.default.yiji,
            data:{parent_id:self.data.cat_id},
    ...
    catClick: function (index) {
        ...
        {
            wx.navigateTo({
                url: '/pages/list/list?cat_id='+this.data.cat_list[index].id
            })
            return;
        }
    ...
    list: function(cat_id,type){
        ...
        var p = self.data.page || 2
        getApp().request({
            url: getApp().api.default.childList,
            data: { 
                cat_id: cat_id,
                page: p,
            },
            success: function (res) {
                ...
                if (res.code == 1000) {
                    getApp().core.hideLoading();
                    if (res.data.length == 0) is_no_more = true;
